...
| Several parts that only make sense assembled together | **F: Concept Build** | One metaphor holds the whole explanation; the assembly slide is the payoff |
...
## Framework F: Concept Build (5–9 slides)

The "here's what's actually in it" carousel — for explaining what something *is*, when it's made of parts that only make sense once you see them together. Not a list (order doesn't matter) and not a demo (nothing gets built) — the payoff is the assembled picture, not a step completed.

| Slide | Job | Pattern |
|---|---|---|
| 1 — Cover | Establish the scene as a whole, with its pieces visibly incomplete — an empty shelf, an empty hook. The gap is the count; don't state a number. | "[Whole thing] is a [scene]. Here's what's actually in it." |
| 2 to N−2 — One part per slide | Zoom into one piece of the scene per slide. Every part maps onto a concrete object in the *same* scene — no part gets an abstract label the others don't share. | "[Concept] = [the object]. [What it does, in scene terms]." |
| N−1 — Assembly | Pull back to the full scene, every part now visible and interacting at once. This is the save-worthy slide — it should summarize the whole post on its own. | "[Whole thing] = [all parts], together." |
| N — Close | Recap what's now understood, then one CTA. If this is a series opener, name what comes next by pointing back at the scene. | "[Recap line]. [CTA — save, follow, or next-part tease]." |

**Every part must share one category.** The moment one part breaks the scene's logic — a physical-object metaphor where one part is suddenly "domain knowledge" instead of an object — the picture stops assembling in the reader's head and the slide reads as a non-sequitur. Reframe the odd part as an object in the same world before shipping (knowledge becomes "the recipe on the shelf," not "domain expertise").

**Patch lossy mappings with one phrase, not a caveat.** Every metaphor loses some precision against the real thing it stands for. Fix the load-bearing gaps inline — one extra clause on the slide that's actually wrong without it — rather than a disclaimer in the caption nobody reads. Cosmetic imprecision can stand; a gap that would let a viewer walk away with a wrong mental model can't.

*SaaS example:* "An AI agent is a chef in a kitchen." → the chef (brain that decides, hands that act) → the recipe (a skill — knowledge pulled off the shelf only when that job comes up) → the appliance (an MCP server — plugged into somewhere specific, not just "a tool") → full kitchen, chef holding the recipe and reaching for the appliance → "Follow for part 2, where we put the chef to work."

**Failure mode:** stretching the scene past what it can hold. Past ~5 parts most physical metaphors strain (a kitchen runs out of distinct, nameable objects before a taxonomy does) — if the concept has more than five load-bearing parts, it's Framework A or C material, not Concept Build.
